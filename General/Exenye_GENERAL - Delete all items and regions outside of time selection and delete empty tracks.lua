--[[
    Script: Exenye_GENERAL - Delete all items and regions outside of time selection and delete empty tracks
    Author: Exenye
    Description: Crop to time selection, keep original positions and delete empty tracks
    Version: 1.0
...
local DeleteEmptyTracks = true -- true / false - Delete empty tracks (except groups)
...
local function DeleteEmptyTracksFunction()
    -- Go through all tracks from back to front to avoid index corruption
    for t = reaper.CountTracks(0), 1, -1 do
        local track = reaper.GetTrack(0, t-1)
        
        -- Check if track has items
        local itemCount = reaper.CountTrackMediaItems(track)
        
        -- Check if track is a folder/group
        local folderDepth = reaper.GetMediaTrackInfo_Value(track, "I_FOLDERDEPTH")
        local isFolder = folderDepth > 0
        
        -- Delete track only if it's empty AND not a folder
        if itemCount == 0 and not isFolder then
...
-- NEW FUNCTION: Delete empty tracks (except groups/folders)
if DeleteEmptyTracks == true then
    DeleteEmptyTracksFunction()
end
...
reaper.PreventUIRefresh(-1)
reaper.Undo_EndBlock("Crop to time selection - keep original positions + delete empty tracks", -1)
